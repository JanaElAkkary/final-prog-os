...
void *handle_client(void *arg)
{
    ...
    while (attempts < 2 && !authenticated)
    {
        ...
        for (int i = 0; i < enc_user_len; i++)
            printf("%02x", enc_user[i]);
        printf("\n");
        ...
        if (authenticate((char *)dec_user, (char *)dec_pass, role))
        {
            ...
            {
                int msg_type;
                if (SSL_read(ssl, &msg_type, sizeof(int)) <= 0)
                    break;

                if (msg_type == 1)
                {
                    int msg_len;
                    if (SSL_read(ssl, &msg_len, sizeof(int)) <= 0)
                        break;
                ...
                { // Read file
                    int name_len;
                    char filename[256];
                    if (SSL_read(ssl, &name_len, sizeof(int)) <= 0)
                        break;
                    if (SSL_read(ssl, filename, name_len) <= 0)
                        break;
                    filename[name_len] = '\0';
                ...
                { // Edit file
                    ...
                    int content_len;
                    if (SSL_read(ssl, &content_len, sizeof(int)) <= 0)
                        break;

                    char content[BUFFER_SIZE];
                    if (SSL_read(ssl, content, content_len) <= 0)
                        break;
                ...
                { // 999 means "exit" sent from client
                    break;
                }
            }
        }
